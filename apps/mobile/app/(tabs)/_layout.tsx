...
const vectorIconSrc = (name: TabIconName) =>
  AppPlatform.isIOS ? undefined : <NativeTabs.Trigger.VectorIcon family={TabIcons} name={name} />;
  ...
  return (
    <TabBarContext value={{ setIsTabBarHidden }}>
      <NativeTabs
        ...
        {...(AppPlatform.isIOS
          ? {}
          : { blurEffect: 'none' as const, backgroundColor: theme.colors.card })}
        minimizeBehavior={
          AppPlatform.OS === 'iosnew' && minimizeTabBarOnScroll[0] ? 'onScrollDown' : 'never'
        }
        iconColor={{
      ...
      >
        ...
        <NativeTabs.Trigger name="devices">
          <NativeTabs.Trigger.Icon
            renderingMode="template"
            xcasset="tab-devices"
            src={vectorIconSrc('devices')}
          />
          <NativeTabs.Trigger.Label hidden={labelsHidden}>
            {t('tabDevices')}
