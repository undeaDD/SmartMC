...
export const AppPreferencesProvider = ({ children }: { children: ReactNode }) => {
  ...
  const [minimizeTabBarOnScroll, setMinimizeTabBarOnScroll] = useState<boolean | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    (async () => {
      ...
      const storedOpenForaumExternally: boolean = await getStored(
        keys.openForaumExternally,
        defaults.openForaumExternally,
      );
      ...
      const storedHideDashboard: boolean = await getStored(
        keys.hideDashboard,
        defaults.hideDashboard,
      );
      setHideDashboard(storedHideDashboard);

      const storedMinimizeTabBarOnScroll: boolean = await getStored(
        keys.minimizeTabBarOnScroll,
        defaults.minimizeTabBarOnScroll,
      );
      setMinimizeTabBarOnScroll(storedMinimizeTabBarOnScroll);

      const storedHideTabLabels: boolean = await getStored(
        keys.hideTabLabels,
        defaults.hideTabLabels,
      );
      ...
      const storedSkipCartAnimation: boolean = await getStored(
        keys.skipCartAnimation,
        defaults.skipCartAnimation,
      );
      setSkipCartAnimation(storedSkipCartAnimation);

      const storedAccessibilityMode: boolean = await getStored(
        keys.accessibilityMode,
  ...
  const resetAll = useCallback(async () => {
    ...
    await SecureStore.setItemAsync(
      keys.openForaumExternally,
      defaults.openForaumExternally.toString(),
    );

    setOpenShopsExternally(defaults.openShopsExternally);
    await SecureStore.setItemAsync(
      keys.openShopsExternally,
      defaults.openShopsExternally.toString(),
    );
    ...
    await SecureStore.setItemAsync(
      keys.minimizeTabBarOnScroll,
      defaults.minimizeTabBarOnScroll.toString(),
    );
  ...
  const value = useMemo<PreferenceContextType>(
    () => ({
      ...
      systemStyle: [
        systemStyle as SystemStyle,
        setSystemStyle as Dispatch<React.SetStateAction<SystemStyle>>,
      ],
      appLocale: [
        appLocale as AppLocale,
        setAppLocale as Dispatch<React.SetStateAction<AppLocale>>,
      ],
      ...
      openForaumExternally: [
        openForaumExternally as boolean,
        setOpenForaumExternally as Dispatch<React.SetStateAction<boolean>>,
      ],
      openShopsExternally: [
        openShopsExternally as boolean,
        setOpenShopsExternally as Dispatch<React.SetStateAction<boolean>>,
      ],
      hideDashboard: [
        hideDashboard as boolean,
        setHideDashboard as Dispatch<React.SetStateAction<boolean>>,
      ],
      minimizeTabBarOnScroll: [
        minimizeTabBarOnScroll as boolean,
        setMinimizeTabBarOnScroll as Dispatch<React.SetStateAction<boolean>>,
      ],
      hideTabLabels: [
        hideTabLabels as boolean,
        setHideTabLabels as Dispatch<React.SetStateAction<boolean>>,
      ],
      autoOpenLink: [
        autoOpenLink as boolean,
        setAutoOpenLink as Dispatch<React.SetStateAction<boolean>>,
      ],
      skipCartAnimation: [
        skipCartAnimation as boolean,
        setSkipCartAnimation as Dispatch<React.SetStateAction<boolean>>,
      ],
      accessibilityMode: [
        accessibilityMode as boolean,
        setAccessibilityMode as Dispatch<React.SetStateAction<boolean>>,
      ],
      resetAll,
    }),
    [
