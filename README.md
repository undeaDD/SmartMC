...
A Minecraft-native smart home / alarm platform. A mod (Fabric, NeoForge, Quilt, and Forge for legacy modpacks) turns your Minecraft server into the entire backend for a companion phone app — no separate service to host, no central server owned by anyone but the world's own server operator.
...
apps/
  ...
  mod/       The Minecraft mod (Fabric / NeoForge on 1.21.1+, Forge on legacy 1.19.2-1.20.1, multi-version via Stonecutter; Quilt runs the Fabric jar unmodified)
packages/
